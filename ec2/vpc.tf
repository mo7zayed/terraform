...
resource "aws_subnet" "public_us_east_1b" {
  ...
  tags = {
    Name        = "${lower(var.app_name)}-${lower(var.app_environment)}-public-us-east-1b"
    Environment = var.app_environment
  }
}
...
resource "aws_subnet" "private_us_east_1b" {
  ...
  tags = {
    Name        = "${lower(var.app_name)}-${lower(var.app_environment)}-private-us-east-1b"
    Environment = var.app_environment
  }
}
...
resource "aws_eip" "eip_nat_us_east_1b" {
  tags = {
    Name        = "${lower(var.app_name)}-${lower(var.app_environment)}-eip-nat-us-east-1b"
    Environment = var.app_environment
  }
}

resource "aws_nat_gateway" "nat_public_us_east_1b" {
  allocation_id = aws_eip.eip_nat_us_east_1b.id
  subnet_id     = aws_subnet.public_us_east_1b.id

  tags = {
    Name        = "${lower(var.app_name)}-${lower(var.app_environment)}-nat-public-us-east-1b"
    Environment = var.app_environment
  }
...
resource "aws_route_table" "rt_private_us_east_1b" {
  vpc_id = aws_vpc.vpc.id

  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.nat_public_us_east_1b.id
  }

  tags = {
    Name        = "${lower(var.app_name)}-${lower(var.app_environment)}-rt-private-us-east-1b"
    Environment = var.app_environment
  }
}
...
resource "aws_route_table_association" "rta_private_us_east_1b" {
  subnet_id      = aws_subnet.private_us_east_1b.id
  route_table_id = aws_route_table.rt_private_us_east_1b.id
}
